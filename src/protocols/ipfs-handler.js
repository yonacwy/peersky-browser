...
export async function createHandler(ipfsOptions, session) {
  ...
  async function initializeIPFSNode() {
    ...
    // Export node for chat module to access libp2p pubsub
    global.ipfsNode = node;
    
    // Initialize IPFS-based chat immediately
    try {
      const { initChat } = await import("../pages/p2p/chat/p2p.js");
      await initChat(node);
    } catch (err) {
      console.error("[Chat] Failed to initialize IPFS chat:", err);
    }
  }
  ...
  return async function protocolHandler(request) {
    ...
    // Handle key generation for editor compatibility
    if (method === 'POST' && urlObj.searchParams.has('key')) {
      const keyName = urlObj.searchParams.get('key');
      console.log(`[IPFS] Generating directory for: ${keyName}`);
      
      // Create a new directory and return its CID
      const directory = await unixFileSystem.addDirectory();
      const directoryCid = directory.cid;
      
      // Store the directory in IPFS for persistence
      const directoryContent = new TextEncoder().encode('{"created": "' + new Date().toISOString() + '"}');
      await unixFileSystem.addFile(directoryContent, {
        path: '.peersky-created',
        parents: [directory]
      });
      
      return new Response(directoryCid.toString(), {
        status: 200,
        headers: { 'Content-Type': 'text/plain' }
      });
    }
