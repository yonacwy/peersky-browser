...
async function setupProtocols(session) {
  ...
  // Hyper SDK disabled due to native module incompatibility with Electron 37
  // hyper-sdk@6.2.2 causes SIGSEGV when createSDK() is called
  // But we still need the protocol handler for chat (which uses IPFS pubsub)
  console.log("[Protocols] Hyper protocol enabled for chat only - SDK disabled");
