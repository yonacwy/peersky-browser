...
async function getDraftDriveUrl() {
    if (!draftDriveUrl) {
        const response = await fetch(`ipfs://localhost/?key=${encodeURIComponent(DRAFT_DRIVE_NAME)}`, { method: 'POST' });
        if (!response.ok) {
            throw new Error(`Failed to generate IPFS key: ${response.statusText}`);
        }
...
async function getOrCreateHyperdrive() {
    if (!hyperdriveUrl) {
        ...
        try {
            const response = await fetch(`ipfs://localhost/?key=${encodeURIComponent(name)}`, { method: 'POST' });
            if (!response.ok) {
                throw new Error(`Failed to generate IPFS key: ${response.statusText}`);
            }
